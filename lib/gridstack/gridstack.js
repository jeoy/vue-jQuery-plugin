...
(function(factory) {
    ...
    GridStackEngine.prototype._fixCollisions = function(node) {
        var self = this;
        this._sortNodes(-1);
        console.log('into _fixCollisions');
        var nn = node;
        ...
        while (true) {
            if (!this.isFixCollisionOn) {
                console.log('because isFixCollisionOn === false return');
                return;
            }
            console.log('into collsion22');
            var collisionNode = _.find(this.nodes, _.bind(Utils._collisionNodeCheck, {node: node, nn: nn}));
            if (typeof collisionNode === 'undefined') {
                console.log('because collisionNode === undefined return');
                return;
            }
            console.log('into MoveNode');
            this.moveNode(collisionNode, collisionNode.x, node.y + node.height,
                collisionNode.width, collisionNode.height, false);
        }
    };
    ...
    GridStackEngine.prototype.moveNode = function(node, x, y, width, height, noPack) {
        ...
        if (node.x == x && node.y == y && node.width == width && node.height == height) {
            console.log('move mid return');
            return node;
        }
        console.log('into real moveNode');
    ...
    GridStack.prototype._prepareElementsByNode = function(el, node) {
        ...
        var onStartMoving = function(event, ui) {
            ...
            self.grid.isFixCollisionOn = false;
            self.container.append(self.placeholder);
            var o = $(this);
            self.grid.cleanNodes();
            self.grid.beginUpdate(node);
        ...
        var onEndMoving = function(event, ui) {
            ...
            if (!o.data('_gridstack_node')) {
                console.log('end Moving unnormal return');
                return;
            }

            self.grid.isFixCollisionOn = true;
            self.grid._fixCollisions(self.grid.currentNode);
